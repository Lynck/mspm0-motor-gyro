 ...
 *
 * 传感器: 八路灰度 (黑线=灯灭=高电平)
 * 硬件: OUT0=PA26, OUT1=PA24, OUT2=PB24, OUT3=PA22,
 ...
 *   while(1) {
 *       delay_ms(5);
 *       LinePatrol_Track(200);  // 速度 200 循迹
 *   }
 */

/* PID 参数 — 可在线调整 */
extern float LinePatrol_Kp;  /* 比例 (默认 3.0) */
extern float LinePatrol_Ki;  /* 积分 (默认 0.01) */
extern float LinePatrol_Kd;  /* 微分 (默认 1.5) */

/* 初始化 GPIO (SysConfig 已配置，内部空) */
void LinePatrol_Init(void);

/* 读取 8 路传感器: bit0=OUT0 ... bit7=OUT7 (1=黑线) */
